...
namespace UnityChan
{
	public class SpringManager : MonoBehaviour
	{
		...
		{
		    UpdateParameterStiffnessForce(stiffnessForce,stiffnessCurve);
		    UpdateParameterDragForce(dragForce,dragCurve);
	    ...
	    private void UpdateParameterStiffnessForce(/*string fieldName,*/ float baseValue, AnimationCurve curve)
	    {
	    ...
	    private void UpdateParameterDragForce(/*string fieldName,*/ float baseValue, AnimationCurve curve)
	    {
	        var start = curve.keys[0].time;
	        var end = curve.keys[curve.length - 1].time;
	        //var step	= (end - start) / (springBones.Length - 1);
