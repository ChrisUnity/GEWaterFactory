...
{	
	...
	void SetVisemeToMorphTarget()
	{
	    ...
	    _canSetValues = !_canSetValues;
		for (int i = 0; i < VisemeToBlendTargets.Length; i++)
		{
			if(VisemeToBlendTargets[i] != -1)
			{
			    float tempfloat = skinnedMeshRenderer.GetBlendShapeWeight(VisemeToBlendTargets[i]);
			    int tempVise = VisemeToBlendTargets[i];
			    DOTween.To(() => tempfloat, x => tempfloat = x, frame.Visemes[i] * 100.0f, 0.1f).OnUpdate(
			        delegate
			        {

			            skinnedMeshRenderer.SetBlendShapeWeight(tempVise, tempfloat);
                    }
			    );
